...
[![Version](https://img.shields.io/badge/version-v0.1.0-4D8F91?style=for-the-badge)](https://github.com/Mahavishnu-K/ore-kernel/releases)
...
- **Cross-Language Tooling:** Agents written in Python or any language can seamlessly and securely execute tools written in Rust, Go, Zig, Javascript, Typescript, Python or C/C++ via pre-compiled `.wasm` cartridges. You no longer have to write tools in the same language as your agent (unless you want to, of course 😼).
...
A dedicated multi-tenant memory manager built with RAII-based `GpuLease` locks. The scheduler utilizes NVML integration (`NvmlGpuMemoryProvider`) to read physical VRAM state, maintaining a `ModelRegistry` of all active models and a `MemoryAccountant` for budget tracking. It dynamically estimates KV cache requirements (with `ore.toml` overrides), performs real-time admission control to prevent OOM errors, and uses an LRU (Least Recently Used) eviction policy to unload idle models only when the physical VRAM is exhausted. When the `GpuLease` drops out of scope, KV cache reservations are instantly released.
