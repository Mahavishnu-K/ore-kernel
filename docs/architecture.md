...
3. **Sandboxed Tool Execution** - Agents can execute pre-compiled WebAssembly tools, autonomous scripts, or **full Python workspaces** through a Zero-Trust WASM Sandbox. ORE ships with a pre-compiled Python WASI runtime, allowing it to dynamically execute `.py` files, parse `requirements.txt`, and set the `PYTHONPATH` without installing Python on the host. It uses deterministic CPU profiling (50M fuel limit) to prevent infinite loops, and a capability-based file system (`cap-std`) to restrict access to an isolated `/workspace`. The sandbox execution runs inside a dedicated blocking thread to prevent starving the async runtime. Agents can also bypass the sandbox to run raw host shell commands if explicitly permitted by their manifest (flagged as UNSAFE).
