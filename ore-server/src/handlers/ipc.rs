...
pub async fn sys_share_context(
    ...
    let safe_overlap = if c_overlap >= c_size { c_size / 4 } else { c_overlap };
    ...
    println!("-> [SEMANTIC BUS] Text split into {} overlapping windows.", total_blocks);
    println!("-> [SEMANTIC BUS] Ready to process {} blocks. Waking up CPU Embedder...", total_blocks);
    ...
    if !chunks_to_embed.is_empty() {
        ...
        match state.driver.generate_embeddings(SYSTEM_EMBEDDER, chunks_to_embed.clone()).await {
            Ok(vectors) => {
...
pub async fn sys_search_context(
    ...
    println!("-> [SEMANTIC BUS] Retrieving the top {} most relevant memory chunks...", k);

    // Translate the question into Math using the System Embedder
    let query_vector = match state.driver.generate_embeddings(SYSTEM_EMBEDDER, vec![payload.query.clone()]).await {
        Ok(v) => v[0].clone(),
    ...
    let top_results = state.semantic_bus.search_pipe(&payload.target_pipe, &query_vector, k, filter_ref); 
