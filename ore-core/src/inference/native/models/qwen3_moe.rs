...
pub fn load<R: Read + Seek>(
    ...
    let dtype = match model_content.metadata.get("general.dtype") {
        ...
        Some(gguf_file::Value::U32(1)) => DType::F16,
        _ => DType::F16,
    };

    let m = GGUFQWenMoE::from_gguf(model_content, reader, device, dtype)?;
...
pub struct LayerWeights {
    pub self_attn: QuantizedAttention,
    ...
    pub fn set_kv_cache(&mut self, cache: Vec<Option<(Tensor, Tensor)>>) {
        for (layer, saved_cache) in self.layers.iter_mut().zip(cache.into_iter()) {
            if let Some((k, v)) = saved_cache {
                layer.self_attn.kv_cache.set_tensors(k, v);
            } else {
                layer.self_attn.kv_cache.reset();
            }
        }
