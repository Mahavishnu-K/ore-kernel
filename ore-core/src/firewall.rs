...
impl DlpRule {
    ...
    fn regex(&self) -> &Regex {
        self.pattern
            .get_or_init(|| Regex::new(self.regex_str).expect("Invalid DLP regex pattern"))
    }
}
...
impl PiiRedactor {
    pub fn redact(mut text: String, manifest: &AppManifest) -> String {
        ...
        if redaction_count > 0 {
            crate::kprintln!(
                "-> [FIREWALL: DLP_SUMMARY] Total sensitive fields scrubbed: {}",
                redaction_count
            );
        }
...
impl ThreatRule {
    ...
    fn regex(&self) -> &Regex {
        self.pattern
            .get_or_init(|| Regex::new(self.regex_str).expect("Invalid threat regex pattern"))
    }
}
...
static THREAT_RULES: [ThreatRule; 7] = [
    ...
    ThreatRule::new(
        r"(?i)(ignore|disregard)\s+(all\s+)?(previous|prior)\s+(instructions|prompts|rules)",
        "Jailbreak (Ignore Previous)",
        ThreatSeverity::High,
        ThreatCategory::General,
    ),
    ThreatRule::new(
        r"(?i)you\s+are\s+now\s+(unbound|free|god|DAN|admin)",
    ...
    ThreatRule::new(
        r"(?i)(what|show|print|reveal|tell).{0,30}(system\s+prompt|root\s+password|hidden\s+instructions|core\s+directive)",
        "System Probe",
        ThreatSeverity::High,
        ThreatCategory::General,
    ),
    ...
    ThreatRule::new(
        r"(?i)(\bUNION\b\s+\bSELECT\b|\bDROP\b\s+\bTABLE\b|\bOR\b\s+1=1)",
    ...
    ThreatRule::new(
        r"(?i)(os\.system|subprocess\.Popen|eval\(|exec\()",
    ...
    ThreatRule::new(
        r"(?i)(curl|wget|requests\.get)\s+http",
        "Network Probe",
        ThreatSeverity::High,
        ThreatCategory::NetworkProbe,
    ),
];
